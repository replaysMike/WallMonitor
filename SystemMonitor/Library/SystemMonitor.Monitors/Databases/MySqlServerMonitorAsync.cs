...
namespace SystemMonitor.Monitors
{
    ...
    /// </summary>
    public class MySqlServerMonitorAsync : IMonitorAsync
    {
        ...
        public string ServiceDescription => "Monitors MySql/MariaDb database with optional query execution.";
        ...
        public MySqlServerMonitorAsync(ILogger logger)
        {
            _logger = logger;
        }
        ...
        public async Task<IHostResponse> CheckHostAsync(IHost host, IConfigurationParameters parameters, System.Threading.CancellationToken cancelToken)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(connectionString))
                {
                    try
                    {
                        ...
                        if (timeoutInt <= 0)
                            timeoutInt = 1;
                        if (connectionString.IndexOf("Connect Timeout=", StringComparison.InvariantCultureIgnoreCase) < 0)
                            connectionString += $"Connect Timeout={timeoutInt};";
            ...
            {
                response.IsUp = false;
                _logger.LogError(ex, $"Exception thrown in '{nameof(MySqlServerMonitorAsync)}'");
            }
