    ...
    public partial class ConsoleWindow : UserControl
    {
        ...
        public ConsoleWindow()
        {
            ...
            if (Design.IsDesignMode)
            {
                ...
                context.AddHistory($"Welcome to System Monitor v{Versioning.GetCurrentVersion()}", ConsoleLogLevel.None, false);
                context.AddHistory("Type /help for a list of commands.", ConsoleLogLevel.None, false);
        ...
        private void RunCommand(string command)
        {
            ...
            switch (parameters[0].ToLower().Replace("/", ""))
            {
                ...
                case "version":
                    try
                    {
                        ViewModel.AddHistory($"Version {Versioning.GetCurrentVersion()}", ConsoleLogLevel.None);
                    }
                    catch (Exception)
                    {
                ...
                case "zoom":
                    if (parameters.Length > 1)
                    {
                        if (parameters[1].ToLower() == "in")
                        {
                            if (ViewModel.FontSize < 32)
                                ViewModel.FontSize += 2;
                            else
                                AudioService.Instance.PlayCancel();
                        }
                        else if (parameters[1].ToLower() == "out")
                        {
                            if (ViewModel.FontSize > 6)
                                ViewModel.FontSize -= 2;
                            else
                                AudioService.Instance.PlayCancel();
                        }
...
/zoom [in/out]                  Increase/decrease console output font size
/clear                          Clear the log
/quit                           Quit
